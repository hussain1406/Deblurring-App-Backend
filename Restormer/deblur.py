...
weights = os.path.join(os.getcwd(),'Restormer','Motion_Deblurring', 'pretrained_models', 'motion_deblurring.pth')
parameters = {'inp_channels':3, 'out_channels':3, 'dim':48, 'num_blocks':[4,6,6,8], 'num_refinement_blocks':4, 'heads':[1,2,4,8], 'ffn_expansion_factor':2.66, 'bias':False, 'LayerNorm_type':'WithBias', 'dual_pixel_task':False}
"basicsr/"
load_arch = run_path(os.path.join(os.getcwd(),"Restormer",'basicsr', 'models', 'archs', 'restormer_arch.py'))
...
print(f"\n ==> Running Motion Deblurring model with weights {weights}\n ")
with torch.no_grad():
